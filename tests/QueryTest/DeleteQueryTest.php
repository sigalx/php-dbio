...
class DeleteQueryTest extends \PHPUnit\Framework\TestCase
{
    public function testTest1()
    {
        ...
        $this->expectExceptionMessage('Missing update condition in safe mode');

        $query = new \sigalx\dbio\Query\DeleteQuery('example_table');
        $query->getSql();
    }
    ...
    public function testTest2()
    {
        $query = new \sigalx\dbio\Query\DeleteQuery('example_table');
        $query->setSafe(false);
        $this->assertEquals('DELETE FROM `example_table`', $query->getSql());
    }
